...
<xsl:template match="sect1">
	    ...
	    <!-- sioc1 Microdata for Preface page -->
    <div style="display:none">
      <div itemscope="" itemtype="http://schema.org/TechArticle">
        ...
        <meta itemprop="name"><xsl:attribute name="content"><xsl:value-of select="title"/></xsl:attribute></meta>
  	    ...
  	    <xsl:call-template name="set-image-mdata-preface" />
        ...
        <link itemprop="isPartOf"><xsl:attribute name="href"><xsl:value-of select="$serveraddr"/>/preface.html</xsl:attribute></link>
      </div>
    </div>
   <xsl:apply-templates />
<br />
...
<xsl:template name="set-image-mdata-preface">
  <xsl:choose>
...
<xsl:template match="sect5" mode="link-href"><xsl:value-of select="../../../../@id"/>.html#<xsl:value-of select="./@id"/></xsl:template>
...
<tr><td><BR/>Figure:
<xsl:variable name="tnt" select="count(parent::*/preceding-sibling::*/figure)+1"/>
<xsl:choose>
  <xsl:when test="parent::*/parent::itemizedlist">
    <xsl:call-template name="pos-itemlist"/><xsl:value-of select="$tnt"/>.
  </xsl:when>
  <xsl:when test="parent::*/parent::orderedlist">
    <!--xsl:if test="'preceding-sibling::listitem/figure'"-->
    <!--/xsl:if-->
    <xsl:call-template name="pos-orderlist"/><xsl:value-of select="$tnt"/>.
  </xsl:when>
  <xsl:otherwise>
    <xsl:call-template name="pos"/>
  </xsl:otherwise>
</xsl:choose> <xsl:value-of select="./title"/></td></tr>
</table>
</xsl:template>
...
<xsl:template name="pos"><xsl:number level="multiple" format=" 1.1.1.1.1.1. " count="chapter|sect1|sect2|sect3|sect4|sect5|figure|table" /></xsl:template>

<!--xsl:template name="pos-itemlist"><xsl:number  level="multiple" format=" 1.1.1.1.1." count="chapter|sect1|sect2|sect3|sect4|itemizedlist"/></xsl:template>
<xsl:template name="pos-orderlist"><xsl:number level="multiple" format=" 1.1.1.1.1." count="chapter|sect1|sect2|sect3|sect4|orderedlist" /></xsl:template-->

<!--xsl:template name="pos-itemlist"><xsl:number  level="multiple" format=" 1.1.1.1.1." count="chapter|sect1|sect2|sect3|sect4|figure"/></xsl:template>
<xsl:template name="pos-orderlist"><xsl:number level="multiple" format=" 1.1.1.1.1." count="chapter|sect1|sect2|sect3|sect4|figure" /></xsl:template-->

<xsl:template name="pos-itemlist"><xsl:number  level="multiple" format=" 1.1.1.1.1.1." count="chapter|sect1|sect2|sect3|sect4|sect5|itemizedlist"/></xsl:template>
<xsl:template name="pos-orderlist"><xsl:number level="multiple" format=" 1.1.1.1.1.1." count="chapter|sect1|sect2|sect3|sect4|sect5|orderedlist" /></xsl:template>
