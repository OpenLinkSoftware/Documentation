...
create procedure db.dba.tdh_fix_block(in pos integer, in elname varchar, in elcontent any, in elwhole any)
{
  ...
  declare normcontent long varchar;
 ...
 normcontent := replace(normcontent, '&lt;?xml version=&quot;1.0&quot; encoding=&quot;UTF-8&quot; ?&gt;', '');    -- fix local xml encoding issues
 normcontent := replace(normcontent, '<?xml version="1.0" encoding="UTF-8" ?>', '');    -- fix local xml encoding issues
 normcontent := replace(normcontent, ' xmlns="http://docbook.org/ns/docbook"', '');     -- fix local xmlns decl issues
...
dbg_printf('TDH: Searching for a %s with hash [%s]', elname, shasum);
 ...
 dbg_printf('TDH: miss!');
 insert into tdh_block_data values (pos, elname, shasum, elcontent);
 return elcontent;
}
;
...
select xslt_stale('file:DocBook/docbook-fix-block-elements.xslt');

dbg_printf('TDH: learning from authoritative document');
xslt('file:DocBook/docbook-fix-block-elements.xslt', xtree_doc(file_to_string_output('./alldocs-develop.xml')));
...
string_to_file('./alldocs-fixed.xml',
       serialize_to_UTF8_xml(xslt('file:DocBook/docbook-fix-block-elements.xslt',
