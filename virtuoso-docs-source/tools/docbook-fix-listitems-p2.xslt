...
<xsl:stylesheet
  ...
  <xsl:template match="//listitem" mode="copy" priority="100">
    <xsl:copy-of select="vi:tdh_fix_block(count(ancestor::*), local-name(.), string(.), .)" />
    <xsl:comment>Check this 1</xsl:comment>
    <!-- <xsl:copy-of select="vi:tdh_fix_block(count(ancestor::*), local-name(.), string(.), .)" /> -->
    <!-- <xsl:element name="{local-name(.)}"> -->
    <!--   <xsl:comment>Check this 2</xsl:comment> -->
    <!--   <xsl:apply-templates select="@*" mode="copy"/> -->
    <!--   <xsl:apply-templates select="vi:tdh_fix_block(count(ancestor::*), local-name(.), string(.), .)" mode="copy" /> -->
    <!-- </xsl:element>  -->
  </xsl:template>
