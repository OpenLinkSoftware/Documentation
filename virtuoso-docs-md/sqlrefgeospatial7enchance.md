...
- <a href="fn_st_transform.html" class="link" title="ST_Transform"><code
  ...
  plugin named proj4, as described below. When the plugin is loaded,
  functions like
  <a href="fn_st_intersects.html" class="link" title="st_intersects"><code
...
The Virtuoso proj4 hosted plugin module is required for performing
...
##### Compiling open source proj4 plugin
...
The proj4 is currently available in the default develop/7 branch of the
"<a href="https://github.com/openlink/virtuoso-opensource" class="ulink"
target="_top">Virtuoso Open Source</a> " git repository and can be build
as follows:
...
installation of enabling the proj4 plugin library to be built in
~/libsrc/plugin/.libs:

``` programlisting
ls libsrc/plugin/.libs/proj4*
...
##### Installation and Configuration of proj4 plugin
...
When the plugin (proj4.so) is built, it needs to be added to the
...
...
21:30:28 PL LOG: Initial setup of DB.DBA.SYS_PROJ4_SRIDS data from
...
1.  `DB.DBA.PROJ4_LOAD_SYS_SRIDS` : it is called at server startup if
    the proj4 plugin is loaded:
...
2.  `DB.DBA.PROJ4_LOAD_INIT_FILE` : it is lower-level procedure:
...
`DB.DBA.SYS_PROJ4_SRIDS table` (the table it is readable for public and
writable for DBA only). When the table is edited, call function
...
The proj4 plugin automatically applies `RAD_TO_DEG` multiplier before
