...
The `username` and `password` optional parameters are used to bind LDAP
connections using basic authentication. If the operation is permitted to
all users then you can leave these parameters out.
...
7.  Apply the virt\_back.sql script against a running virtuoso server,
    for example with isql.

8.  If you can also apply the virt\_testdb.sql to insert some demo
    schema and entries into the DB.
