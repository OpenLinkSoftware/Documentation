...
<xsl:stylesheet
  ...
  <xsl:param name="previewlength"/>
  <xsl:param name="mimetype"/>
  <xsl:param name="source"/>
  <xsl:param name="furthercontributors" />
  <xsl:param name="source_mime" />
  ...
  <xsl:template match="html/head">
    <rdf:Description rdf:about="{$docproxyIRI}">
      <rdf:type rdf:resource="&schema;WebPage"/>
      <xsl:apply-templates select="//h:head/h:meta[@http-equiv='Content-Type']"/>
      <dc:title>
        <xsl:value-of select="$baseUri"/>
      </dc:title>
    </rdf:Description>
    <rdf:Description rdf:about="{$resourceURL}">
      <xsl:if test="not(contains($mimetype,'rdf'))">
        <owl:sameAs rdf:resource="{$docIRI}"/>
      </xsl:if>
      <xsl:if test="$furthercontributors=0">
        <rdf:type rdf:resource="&schema;CreativeWork" />
      </xsl:if>
      ...
      <!--<rdf:value><xsl:value-of select="normalize-space(vi:html2text($doc1))" /></rdf:value>-->
      <xsl:if test="not ($baseUri like 'http://%.nytimes.com/%' or $baseUri like 'http://stackoverflow.com/%' or $baseUri like 'http://%.stackexchange.com/%' or $baseUri like 'http%://%.wolframalpha.com/%')">
	...
	<awol:type><xsl:value-of select="$source_mime"/></awol:type>
    ...
    <xsl:for-each select="//*[starts-with (., '#X509Cert Fingerprint:')]">
      <xsl:variable name="fp">
        <xsl:value-of select="substring-before (substring-after (., '#X509Cert Fingerprint:'), ' ')"/>
      </xsl:variable>
      <xsl:variable name="fpn">
        <xsl:value-of select="translate ($fp, ':', '')"/>
      </xsl:variable>
    ...
    <xsl:for-each select="//*[text() like '%di:%?hashtag=webid%']">
      <xsl:variable name="di">
        <xsl:copy-of select="vi:di-split (.)"/>
      </xsl:variable>
      <xsl:variable name="au">
        <xsl:choose>
          <xsl:when test="//link[@rel='canonical']/@href"><xsl:value-of select="//link[@rel='canonical']/@href"/>#author</xsl:when>
      ...
      <xsl:for-each select="$di/result/di">
        ...
        <xsl:variable name="ct">
          <xsl:value-of select="vi:proxyIRI ($baseUri,'',$fp)"/>
        </xsl:variable>
        <foaf:Person rdf:about="{$au}">
          <oplcert:hasCertificate rdf:resource="{$ct}"/>
        </foaf:Person>
    ...
    <xsl:if test="not ($url like 'javascript:%')">
      <xsl:choose>
        <xsl:when test="$url like 'http://www.amazon.com/gp/product/%' or $url like 'http://www.amazon.%/o/ASIN/%'">
          <xsl:variable name="tmp" select="vi:regexp-match ('(http://www.amazon.com/gp/product/[A-Z0-9]+)|(http://www.amazon.[^/]*/o/ASIN/[A-Z0-9]+)', $url)"/>
  ...
  <xsl:template match="head|script|style|form|input|button|textarea|object|frame|frameset|select" mode="content"/>
  ...
  <xsl:template match="*[count(*)=0]" mode="textonly" priority="3">
    <xsl:value-of select="normalize-space(./text())"/>
    <xsl:text> 
</xsl:text>
  </xsl:template>
  
  <xsl:template match="@*|*:script|*:style|*:img|*:form|*:object|*:iframe|*:nav|*:footer|*:header" mode="textonly" priority="4"/>
