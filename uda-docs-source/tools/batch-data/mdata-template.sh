...
urlbase="$1"
[ -z "$urlbase" ] && urlbase="http://docs.openlinksw.com/uda/"
...
basekeywords="OpenLink,OpenLink Software,database,RDBMS,relational,SQL"
class="http://schema.org/WebPage"
...
(cd $OUTDIR && find ee mt lite -iname \*.html) | while read f
do
    url="$urlbase$f"
