...
<xsl:stylesheet
  ...
  <xsl:output encoding="UTF-8" indent="yes" method="xml"
              ...
              exclude-result-prefixes="db Comps Demographics SearchResults
                                       ...
                                       xfn xfnv xhv xi xlink xn xsd xsi xsl10
  ...
  <!-- Replace figure/title with image filename where empty -->
  <!-- <xsl:template match="*[local-name(.)='title' and local-name(..)='figure' -->
  <!--                      and count(./text())=0]" mode="copy" priority="10"> -->
  <!--   <xsl:element name="title"> -->
  <!--     <xsl:value-of select="..//*/@*[name(.)='fileref'][1]"/> -->
  <!--   </xsl:element> -->
  <!-- </xsl:template> -->

  <!-- Add figure/title where missing -->
  <!-- <xsl:template match="*[name()='figure' and count(*[name(.)='title'])=0]" mode="copy" priority="10"> -->
  <!--   <xsl:element name="figure"> -->
  <!--     <xsl:apply-templates select="@*" mode="copy" /> -->
  <!--     <title><xsl:value-of select=".//@fileref[1]"/> -->
  <!--     </title> -->
  <!--     <xsl:apply-templates mode="copy"/> -->
  <!--   </xsl:element> -->
  <!-- </xsl:template> -->

  <!-- formalpara with multiple child para -->
  <xsl:template match="*[name(.)='formalpara' and count(*[name(.)='para'])&gt;1 and *[name(.)='title']]" mode="copy" priority="10">
  ...
  <!-- better attempt at fixing listitem/text() directly -->
  <!-- <xsl:template -->
  <!--     match="*[name(.)='listitem']/text()[normalize-space(.)!='']" -->
  <!--     mode="copy"> -->
  <!--   <xsl:element name="para"> -->
  <!--     <xsl:copy-of select="."/> -->
  <!--   </xsl:element> -->
